...
describe('Guvera test', function () {

  var modalNumber = element.all(by.tagName('span')).get(1);
  var modalButton = element(by.tagName('button'));
  var loginInput = element(by.css('[placeholder="Email / Username"]'));
  var hideButton = element(by.css('[ng-click="$hide()"]'));
  var justSomeText = element(by.tagName('h2'));

  beforeEach(function() {
    browser.get('https://www.guvera.com/login');
  });

  it('login to Guvera', function () {
    loginInput.click();
    loginInput.sendKeys("testing 1234")
    expect(modalNumber.getText()).toEqual('1');
  });
